...
def get_data(dataset, max_train_size=None, max_test_size=None, print_log=True, do_preprocess=True, train_start=0,
    ...
    for filename in os.listdir(prefix):
        if filename.endswith('train.pkl'):
            with open(os.path.join(prefix, filename), "rb") as lettura:
                content = pickle.load(lettura)
            db_train.append(content)

    myfile = open(os.path.join(prefix, "Exa_train.pkl"), "wb")
    pickle.dump(db_train, myfile, protocol=pickle.HIGHEST_PROTOCOL)
    myfile.close()
    ...
    for filename in os.listdir(prefix):
        if filename.endswith('test.pkl'):
            with open(os.path.join(prefix, filename), "rb") as lettura:
                content = pickle.load(lettura)
            db_test.append(content)

    myfile = open(os.path.join(prefix, "Exa_test.pkl"), "wb")
    pickle.dump(db_test, myfile, protocol=pickle.HIGHEST_PROTOCOL)
    myfile.close()

    db_test_label = []
    for filename in os.listdir(prefix):
        if filename.endswith('test_label.pkl'):
            with open(os.path.join(prefix, filename), "rb") as lettura:
                content = pickle.load(lettura)
            db_test_label.append(content)
